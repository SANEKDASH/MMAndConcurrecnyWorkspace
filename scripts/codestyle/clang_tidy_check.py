...
def get_args():
    parser = argparse.ArgumentParser(
        description="Runner for clang-tidy for project project.")
    parser.add_argument(
        'project_dir', help='project sources directory.', type=str)
    parser.add_argument(
        'build_dir', help='project build directory.', type=str)
    parser.add_argument(
...
def run_clang_tidy(src_path: str, project_dir: str, build_dir: str, compile_args: str) -> bool:
    ...
    cmd += ['--header-filter=.*']
    cmd += ['--config-file=' + os.path.join(project_dir, '.clang-tidy')]
    cmd += [src_path]
    cmd += ['--']
...
def get_full_path(relative_path: str, location_base: str, project_dir: str, build_dir: str) -> str:
    full_path = project_dir if location_base == "project_DIR" else build_dir
    full_path += "/" + relative_path
    full_path = str(os.path.realpath(full_path))
    return full_path


def check_file_list(file_list: list, project_dir: str, build_dir: str, proc_count: int) -> bool:
    ...
    for src, args in file_list:
        ...
        proc = pool.apply_async(func=run_clang_tidy, args=(
            src, project_dir, build_dir, args))
...
def need_to_ignore_file(file_path: str, project_dir: str, build_dir: str) -> bool:
...
def get_file_list(project_dir: str, build_dir: str, filename_filter: str) -> list:
    ...
    for cmd in cmds_json:
        ...
        if need_to_ignore_file(file_path_raw, project_dir, build_dir):
            continue

        file_path = str(os.path.realpath(cmd["file"]))
        if need_to_ignore_file(file_path, project_dir, build_dir):
            continue
...
def filter_file_duplicated_options(file_list: list) -> list:
    filtered = []
    regexp = re.compile(".*Dproject_ENABLE_RELAYOUT_PROFILE.*")
    for file_path, keys in file_list:
...
def verify_args(project_dir: str, build_dir: str) -> str:
...
def check_headers_in_es2project_sources(project_dir):
    es2project_dir = os.path.join(project_dir, "tools/es2project")
    result = []
    for root, dirs, files in os.walk(es2project_dir):
        for file in files:
...
if __name__ == "__main__":
    ...
    err_msg = verify_args(arguments.project_dir, arguments.build_dir)
    if err_msg:
        sys.exit(err_msg)

    files_list = get_file_list(
        arguments.project_dir, arguments.build_dir, arguments.filename_filter)
    ...
    check_headers_in_es2project_sources(arguments.project_dir)
    ...
    if not check_file_list(files_list, arguments.project_dir, arguments.build_dir, process_count):
